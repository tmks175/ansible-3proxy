...
5. Проверить статус службы на целевом сервере:
`systemctl status 3proxy`
6. Порт и учетные данные для подключения находятся в директории *(генерируются автоматически в процессе выполнения playbook; при повторном запуске - не изменяются):*
...
Для автоматического сохранения и восстановления правил после перезагрузки сервера, рекомендуется установить пакет: \
`iptables-persistent`

---

**Опционально:** версию 3proxy и часовой пояс можно изменить в переменной: \
`vars` - `/ansible-3proxy/playbook.yml`
